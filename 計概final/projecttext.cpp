...
class Control:public People,public Elevator {
	public:
		void move_ele(int first_y, int after_y,int select,int size,bool type,int time,int x,int non_y1,int non_y2);
		//移動電梯 初始y軸跟後來y軸 x為三台電梯的其中一個x select用來選擇使用量
		//初始size跟看是第一次還是第二次 如果第一次為true 並呼叫getFst 反之為false 呼叫getSecFloor
		//select1_x為電梯1的x值 因為人的位置是在第一個電梯旁
		//time為電梯的開門時間
...
int main() {
    ...
    int people_floor[3][150];
	int people_floor_sec[3][150];
	for (int i = 0; i < 3;i++)
    {
        for (int j = 0; j < 3; j++)
        {
    ...
    people_floor[1][0] = 1;
    people_floor[2][0] = 1;
    people_floor_sec[0][0] = 1;
    people_floor_sec[1][0] = 1;
    people_floor_sec[2][0] = 1;
    int first_1=0,first_2=0,first_3=0;
    int second_1=0,second_2=0,second_3=0;
	int oldfirst_1, oldfirst_2, oldfirst_3;
	int oldsecond_1, oldsecond_2, oldsecond_3;
	while (true)
    {
        for (int j = 0; j < 12; j++)
        {
		...
		int newsize = control.getSize();
		oldfirst_1=first_1,oldfirst_2=first_2 ,oldfirst_3=first_3 ;
		int n = 1; //產生的人數 用來move 人的x
        for (int i = oldsize; i < control.getSize(); i++)
        { //每個人第一次選擇電梯的情況 統計使用量與人數
            if (control.assign(control.getFst(i), first) == 0)
            { //一開始選擇電梯的情況
                select_ele[0] = true;
                control.Add(0, 1);    //是第一個電梯人數就加1
                elein_peo_count[0]++; //代表第一電梯有人使用
				++first_1;
                people_floor[0][first_1] = control.getFst(i);
            ...
            else if (control.assign(control.getFst(i), first) == 1)
            {
                ...
                elein_peo_count[1]++;
				++first_2;
				people_floor[1][first_2] = control.getFst (i);
            }
            else if (control.assign(control.getFst(i), first) == 2)
            {
                ...
                elein_peo_count[2]++;
				++first_3;
				people_floor[2][first_3] = control.getFst(i);
            }
            for (int j = 0; j < 12; j++)
            { //計算要去那樓層的人有幾人
		...
		control.move_elevator(select_ele[0], select_ele[1], select_ele[2], people_floor_sec[0][second_1], 1, people_floor_sec[1][second_2], 1, people_floor_sec[2][second_3], 1);

		sort(people_floor[0] + oldfirst_1, people_floor[0] + first_1);
		sort(people_floor[1] + oldfirst_2, people_floor[1] + first_2);
		sort(people_floor[2] + oldfirst_3, people_floor[2] + first_3);
		Sleep(2000); //單位時間為0.5秒
        first = false;
		clear();
        control.Floor();
        refresh();
        if (elein_peo_count[0] != 0)
        {                                            //代表有人要使用第一個電梯 move 跟 cout
            control.waitTime(0, elein_peo_count[0]); //計算等待時間
            int time = control.getwait_time(0);      //得到等待時間
            total_waiting_time += time; //把等待時間加入
        }
        else if (elein_peo_count[1] != 0)
        { //代表有人要使用第二個電梯
            control.waitTime(1, elein_peo_count[1]);
            int time = control.getwait_time(1);
            total_waiting_time += time;
        }
        else if (elein_peo_count[2] != 0)
        { //代表有人要使用第三個電梯
            control.waitTime(2, elein_peo_count[2]);
            int time = control.getwait_time(2);
            total_waiting_time += time;
        }
		//control.move_elevator(select_ele[0], select_ele[1], select_ele[2],1,people_floor[0][oldfirst_1],1, people_floor[1][oldfirst_2],1, people_floor[2][oldfirst_3]);
		control.move_elevator(select_ele[0], select_ele[1], select_ele[2], people_floor[0][oldfirst_1], people_floor[0][first_1], people_floor[1][oldfirst_2], people_floor[1][first_2], people_floor[2][oldfirst_3],people_floor[2][first_3]);
		 //int second_1=0,second_2=0,second_3=0;
        for (int i = 0; i < 3; i++)
        {
            select_ele[i] = false;
        }
		oldsecond_1 = second_1;
		oldsecond_2 = second_2;
		oldsecond_3 = second_3;
        ...
        {                        //第二次選電梯的情況 統計用
            ...
            { //後來選擇電梯的情況
                ...
                select_ele[0] = true;
				++second_1;
				people_floor_sec[0][second_1] = control.getSecFloor(i);
            ...
            else if (control.assign(control.getSecFloor(i), first) == 1)
            {
                ...
                select_ele[1] = true;
				++second_2;
				people_floor_sec[1][second_2] = control.getSecFloor(i);
            ...
            else if (control.assign(control.getSecFloor(i), first) == 2)
            {
                ...
                select_ele[2] = true;
				++second_3;
				people_floor_sec[2][second_3] == control.getSecFloor(i);
				/*if(control.getSecFloor(i)==1)
					control.minus(2,1);*/
            }
            for (int j = 0; j < 12; j++)
		...
		sort(people_floor_sec[0] + oldsecond_1, people_floor_sec[0] + second_1);
		sort(people_floor_sec[1] + oldsecond_2, people_floor_sec[1] + second_2);
		sort(people_floor_sec[2] + oldsecond_3, people_floor_sec[2] + second_3);
		control.move_elevator(select_ele[0], select_ele[1], select_ele[2], people_floor[0][first_1], people_floor_sec[0][oldsecond_1],people_floor[1][first_2],people_floor_sec[1][oldsecond_2],people_floor[2][first_3], people_floor_sec[2][oldsecond_3]);
		control.move_elevator(select_ele[0], select_ele[1], select_ele[2], people_floor_sec[0][oldsecond_1], people_floor_sec[0][second_1],people_floor_sec[1][oldsecond_2],people_floor_sec[1][second_2],people_floor_sec[2][oldsecond_3], people_floor_sec[2][second_3]);
